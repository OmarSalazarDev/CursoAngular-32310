...
export class TablaComponent implements OnInit {
  ...
  editar(elemento: Curso) {
    const dialoRef = this.dialog.open(EditarDialogComponent, {
      width: '350px',
      data: elemento
    });

    dialoRef.afterClosed().subscribe(resultado => {
      if (resultado) {
  ...
  filtrar(event: Event) {
    const calorObtenido = (event.target as HTMLInputElement).value;
    // this.dataSource.filter = calorObtenido.trim().toLowerCase();
    this.dataSource.filter = calorObtenido.trim().toLocaleLowerCase();
  }
